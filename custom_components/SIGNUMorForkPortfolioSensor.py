...
class SIGNUMorForkPortfolioSensor(SensorEntity):
    """Representation of an SignumPortfolio sensor."""

    def __init__(self, main_coin, explorer_api_url, name, address, token, token_address, decimals):
    ...
    def fetch_value(self, params):
        ...
        data = r.json()
        return int(data['balanceNQT']) / (10 ** self._decimals)
    ...
    def update(self):
        ...
        if self._token_address:
            ...
            self._state = self.fetch_value({'requestType': 'getBalance', 'account': self._address,
                                            })
